...
def compute_distance(point1, point2):
	lon1, lat1, lon2, lat2 = [math.radians(g) for g in (point1[1], point1[0], point2[1], point2[0])]
...
def tag_highway(segment, lanes, tags, extras):
	...
	if segment['typeVeg'] in ["Kanalisert veg", "Enkel bilveg"] and \
			"strekning" in segment['vegsystemreferanse'] and segment['vegsystemreferanse']['strekning'][
		'trafikantgruppe'] == "G" or \
			segment['typeVeg'] == "Gang- og sykkelveg" and "topologinivå" in segment and segment[
		'topologinivå'] == "KJOREBANE":
	...
	# Tagging for normal highways (cars)
	...
	elif segment['typeVeg'] in ["Enkel bilveg", "Kanalisert veg", "Rampe",
								"Rundkjøring"]:  # Regular highways (excluding "kjørefelt")

		if "sideanlegg" in segment['vegsystemreferanse'] or \
				len(lanes) == 1 and "K" in lanes[0] and ref['vegkategori'] in ["E", "R", "F"] and segment[
			'detaljnivå'] != "Kjørebane" and \
				(segment['typeVeg'] != "Enkel bilveg" or "kryssystem" in segment[
					'vegsystemreferanse']):  # Trafikklommer/rasteplasser
		...
		if segment['detaljnivå'] == "Kjørefelt" and not (
				lanes and ("K" in lanes[0] and lanes[0] != "SVKL")):  # or "H" in lanes[0])):
			#			tags.clear()
	...
	if debug:
		...
		extras["KATEGORI"] = "#" + ref['vegsystem']['vegkategori'] + " " + \
							 road_category[ref['vegsystem']['vegkategori']]['name']

		if "sideanlegg" in ref:
			extras['SIDEANLEGG'] = "{}-{} id:{}".format(
			ref['sideanlegg']['sideanlegg'], ref['sideanlegg']['sideanleggsdel'], ref['sideanlegg']['id'])
		if "kryssystem" in ref:
			extras['KRYSSYSTEM'] = "{}-{} id:{}".format(
			ref['kryssystem']['kryssystem'], ref['kryssystem']['kryssdel'], ref['kryssystem']['id'])
	...
	if object_id == "595":
		...
		if "Trafikkreguleringer" in properties:
			if properties['Trafikkreguleringer'].strip() in restrictions:
				...
				message("  *** Unknown access restriction: {}\n".format(properties['Trafikkreguleringer']))
		...
		if "Vinterstengt, fra dato" in properties or "Vinterstengt, til dato" in properties:
			...
			if "Vinterstengt, fra dato" in properties and "Vinterstengt, til dato" in properties:
				tags['motor_vehicle:conditional'] = "no @ {}-{}".format(
				calendar.month_abbr[int(properties['Vinterstengt, fra dato'][0:2])],
...
def clip_segment(segment, clip_position):
	...
	if clip_position == segment['parent_start'] or clip_position == segment['parent_end']:
		message("  *** Too short clipping: {} {:f}\n".format(segment['id'], clip_position))

	clip_length = (clip_position - segment['parent_start']) * segment['length'] / (
				segment['parent_end'] - segment['parent_start'])  # In meters
	...
	node_id = create_new_node("", new_node, {segment['id'], new_segment_id})
...
def update_segments_line(parent_sequence_id, tag_start, tag_end, direction, new_tags, new_extras):
	...
	if "tunnel" in new_tags or "bridge" in new_tags:
		for segment_id in parents[parent_sequence_id][:]:
			...
			if ("tunnel" in new_tags and "tunnel" in segment['tags'] or "bridge" in new_tags and "bridge" in segment[
				'tags']) and \
...
def insert_node(segment, insert_position):
	position_length = (insert_position - segment['parent_start']) * segment['length'] / (
				segment['parent_end'] - segment['parent_start'])  # In meters
	...
	if segment['geometry'][j][2] and position_length - previous_length < point_margin \
		...
		if previous_length + node_length - position_length < position_length - previous_length and \
				segment['geometry'][j + 1][2] or not segment['geometry'][j][2]:
...
def update_segments_point(parent_sequence_id, tag_position, new_tags, new_extras):
	for segment_id in parents[parent_sequence_id]:
		...
		if segment['parent_start'] <= tag_position <= segment[
			'parent_end']:  # and (not side or side == "H" and not segment['reverse'] or side == "V" and segment['reverse']):
			position = tag_position

			if "highway" in new_tags and new_tags[
				'highway'] == "traffic_signals":  # Traffic signals to snap to closest start/end of segment
...
def traverse_network(segment_id, from_node_id, route, distance, target_segments, max_distance):
	...
	if debug_traverse:
		message("  Traverse: {} {} {} {:f}m\n".format(segment_id, from_node_id, route, distance))
	...
	if "motor_vehicle:forward" in segment['tags'] and (segment['reverse'] and segment['end_node'] == from_node_id or \
													   not segment['reverse'] and segment[
														   'start_node'] == from_node_id) or \
			"motor_vehicle:backward" in segment['tags'] and (
			segment['reverse'] and segment['start_node'] == from_node_id or \
			not segment['reverse'] and segment['end_node'] == from_node_id):
		if debug_traverse:
			message("    No cars in that direction - {}\n".format(segment_id))
		return (False, distance, route)
	...
	if debug_traverse:
		message("    Node ways: {}\n".format(nodes[next_node_id]['ways']))
	...
	for next_segment_id in nodes[next_node_id]['ways']:
		if next_segment_id != segment_id:
			...
			if test_result and test_distance < best_distance:  # and via_node_id in test_route
				...
				if debug_traverse:
					message("    New best route: {} {:f}m\n".format(best_route, best_distance))
...
def create_turn_restriction(restriction, restriction_id):
	...
	found = False
	for turn_restriction_id, turn_restriction in turn_restrictions.items():
...
def get_road_object(object_id, **kwargs):
	message("Merging object type #{} {}...".format(object_id, object_types[object_id]))
		...
		for road_object in data['objekter']:
			...
			for attribute in road_object['egenskaper']:
				if "verdi" in attribute:
					...
					key = "VEGOBJEKT_{}_{}".format(
					object_id, attribute['navn'].replace(" ", "_").replace(".", "").replace(",", "").upper())
				...
				if object_id == "47":  # Meeting place
					for i, location in enumerate(locations):
						if location['stedfestingstype'] == "Linje":
							...
							locations[i]['relativPosisjon'] = (location['startposisjon'] + location[
								'sluttposisjon']) * 0.5
					...
					if len(locations) % 2 == 0:
						...
						mid_location['relativPosisjon'] = (mid_location['startposisjon'] + mid_location[
							'sluttposisjon']) * 0.5
						...
						if location['stedfestingstype'] == "Linje":
							if location['startposisjon'] != location['sluttposisjon']:
								if location['kjørefelt']:
									...
									direction = ""
								update_segments_line(location['veglenkesekvensid'], location['startposisjon'],
													 location['sluttposisjon'], \
													 direction, tags, extras)
						#							else:
						#								message ("  *** Equal start and end positions in location - %i\n" % road_object['id'])

						# For pedestrian crossings, only accept "M" positions
						elif location['stedfestingstype'] == "Punkt" and ("sideposisjon" not in location or location[
							'sideposisjon'] == "M"):  # or object_id == "96"
							update_segments_point(location['veglenkesekvensid'], location['relativPosisjon'], tags,
												  extras)
		#					else:
	...
	message("  {} objects\n".format(total_returned))

	if debug:
		debug_file = open("nvdb_vegobjekt_{}_input.json".format(object_id), "w")
				...
				if tags:
					...
					if sequence['veglenkesekvensid'] in sequences:
						for check_segment_id in sequences[sequence['veglenkesekvensid']]:
							...
							margin = (check_segment['sequence_end'] - check_segment['sequence_start']) / check_segment[
								'length'] * node_margin  # Meters
							if segment['startposisjon'] <= check_segment['sequence_start'] + margin and check_segment[
								'sequence_end'] - margin <= segment['sluttposisjon']:
...
def process_road_network(segment):
	if segment['detaljnivå'] != "Vegtrase":
		...
		if debug:
			...
			extras['NODER'] = "{} {}".format(segment['startnode'], segment['sluttnode'])

			if "superstedfesting" in segment:
				parent = segment['superstedfesting']
				extras['STED_SUPER'] = "{:f}-{:f}@{} {} {}".format(
				parent['startposisjon'], parent['sluttposisjon'], parent['veglenkesekvensid'], parent['retning'],
				parent['sideposisjon'])
			...
			if ref:
				if "strekning" in ref:
					extras['STED_SEGMENT'] = "{} {} ({:.2f}m)".format(
					segment['kortform'], ref['strekning']['retning'], segment['lengde'])
					if "adskilte_løp" in ref['strekning'] and ref['strekning']['adskilte_løp'] != "Nei":
						extras['ADSKILTE_LØP'] = "{} {}".format(
						ref['strekning']['adskilte_løp'], ref['strekning']['adskilte_løp_nummer'])
				else:
					extras['STED_SEGMENT'] = "{} ({:.2f}m)".format(segment['kortform'], segment['lengde'])
			else:
				extras['STED_SEGMENT'] = "{} ({:.2f}m)".format(segment['kortform'], segment['lengde'])
...
def process_road_object(road_object):
		...
		for attribute in road_object['egenskaper']:
			...
			if "verdi" in attribute:
				...
				if attribute['egenskapstype'] == "Stedfesting":
					value = "{:f}@{} {}".format(
					attribute['relativPosisjon'], attribute['veglenkesekvensid'], attribute['retning'])
				...
				value = "{:f}@{}".format(attribute['relativPosisjon'], attribute['veglenkesekvensid'])
		...
		if ("lokasjon" in road_object) and ("stedfestinger" in road_object['lokasjon']):
			...
			for stedfesting in road_object['lokasjon']['stedfestinger']:
				...
				if "lengde" in stedfesting:
					place += " ({.2})".format(stedfesting['lengde'])
				if place:
	...
	for segment in road_object['vegsegmenter']:
		...
		if "sluttdato" not in segment and "strekning" in segment['vegsystemreferanse']:
			geometry, geometry_type = process_geometry(segment['geometri']['wkt'])
			if len(geometry) > 1:
				message("  *** More than one geometry - {}\n".format(road_object['id']))
			geometry = geometry[0]
			highway_type = ""

			if geometry_type == "line":
				highway_type = tag_highway(segment, [], segment_tags, segment_extras)  # No lanes
				segment_extras['STED_SEGMENT'] = "{:f}-{:f}@{} {} ({.2f}m)".format(
				segment['startposisjon'], segment['sluttposisjon'], segment['veglenkesekvensid'],
				segment['vegsystemreferanse']['strekning']['retning'], segment['lengde'])
			elif "relativPosisjon" in segment:
				segment_extras['STED_SEGMENT'] = "{:f}@{} {}".format(
					segment['relativPosisjon'], segment['veglenkesekvensid'],
					segment['vegsystemreferanse']['strekning']['retning'])
			elif "startposisjon" in segment and "sluttposisjon" in segment and segment['startposisjon'] == segment[
				'sluttposisjon']:
				segment_extras['STED_SEGMENT'] = "{:f}@{} {}".format(
					segment['startposisjon'], segment['veglenkesekvensid'],
					segment['vegsystemreferanse']['strekning']['retning'])
...
def output_osm():
	...
	for node_id, node in nodes.items():
		osm_id -= 1
		osm_node = ET.Element("node", id=str(osm_id), action="modify", lat=str(node['point'][0]),
							  lon=str(node['point'][1]))
	...
	for way_segments in ways:
		...
		if segment['geotype'] == "line":  # Way
			...
			if debug or object_tags:
				for key, value in segment['extras'].items():
	...
	for restriction_id, restriction in turn_restrictions.items():
		osm_id -= 1
		...
		osm_relation.append(
			ET.Element("member", type="way", ref=str(segments[restriction['from_segment']]['osmid']), role="from"))
		osm_relation.append(
			ET.Element("member", type="way", ref=str(segments[restriction['to_segment']]['osmid']), role="to"))
		osm_relation.append(
			ET.Element("member", type="node", ref=str(nodes[restriction['via_node']]['osmid']), role="via"))
	...
	message("Saved {} elements in file '{}'\n\n".format(count, output_filename))
	...
	for node_id in [segment['start_node'], segment['end_node']]:
		...
		while check_next:
			...
			for check_segment_id in nodes[last_node_id]['ways']:
				check_segment = segments[check_segment_id]
				if check_segment_id not in checked_segments and checked_segments['highway'] == last_segment['highway']:
...
def optimize_network():
	...
	if longer_ways:
		...
		for sequence_id, sequence_segment in sequences.items():
	...
	for segment_id, segment in segments.items():
		if segment['geotype'] == "line":
			...
			if segment['reverse']:
				segment['start_node'], segment['end_node'] = segment['end_node'], segment[
					'start_node']  # Swap for output
				segment['geometry'].reverse()

			if debug and "direction" in segment:
				segment['extras']['STED_INTERN'] = "{:f}-{:f}@{} {} ({.2f}m)".format(
					segment['parent_start'], segment['parent_end'], segment['parent'],
	...
	if longer_ways:
		...
		for sequence_id, sequence_segments in sequences.items():
...
def get_data(url):
	...
	if debug:
		debug_file = open("nvdb_{}_input.json".format(function), "w")
		debug_file.write("[\n")
	...
	while returned > 0:
		...
		total_returned += returned
		message("\r{}".format(total_returned))
	...
	message("\rDone processing {} road objects/segments\n".format(total_returned))
...
if __name__ == '__main__':
	...
	if len(sys.argv) > 2:
		if (sys.argv[1] == "-vegnett") and len(sys.argv) >= 3 and sys.argv[2].isdigit():
			...
			url = server + "vegobjekter/" + sys.argv[
				2] + "?inkluder=metadata,egenskaper,geometri,lokasjon,vegsegmenter&alle_versjoner=false&srid=wgs84&kommune=" + municipality

		elif (sys.argv[1] == "-vegobjekt") and (len(sys.argv) >= 3) and sys.argv[2].isdigit():
			object_type = sys.argv[2]
			url = server + "vegobjekter/" + sys.argv[
				2] + "?inkluder=metadata,egenskaper,geometri,lokasjon,vegsegmenter&alle_versjoner=false&srid=wgs84"
	...
	if url and municipality:
		if municipality in municipalities:
			message("Municipality:   #{} {}\n".format(municipality, municipalities[municipality]))
			if not object_type:
	...
	if url and object_type:
		if object_type in object_types:
			message("Road object:    #{} {}\n".format(object_type, object_types[object_type]))
			output_filename += "_" + object_type + "_" + object_types[object_type].replace(" ", "_")
		else:
			message("*** Road object type {} not found\n".format(object_type))
			url = ""
	...
	if url:
		...
		message(
			'  nvdb2osm -vegobjekt <nnn> <mmmm>  -->  Road object number (2-3 digits) for municipality number (4 digits)\n')
		message(
			'  nvdb2osm -url "<api url string>"  -->  Any api generated from vegkart.no (UTM bounding box not supported, wgs84 appended)\n')
		sys.exit()
